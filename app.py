...
# Pastikan dataset NLTK sudah terunduh
data_path = os.path.join(os.getcwd(), 'nltk_data')
nltk.data.path.append("C:/Users/ikhsan/AppData/Roaming/nltk_data")
nltk.download('punkt', download_dir="C:/Users/ikhsan/AppData/Roaming/nltk_data")
nltk.download('stopwords', download_dir="C:/Users/ikhsan/AppData/Roaming/nltk_data/corpora")
...
def index():
    similarity_score, classification_result = None, None
    ...
    if request.method == 'POST':
        ...
        if file and (file.filename.endswith('.pdf') or file.filename.endswith('.docx')):
            ...
            if uploaded_text:
                existing_texts = [extract_text_from_pdf(os.path.join(UPLOAD_FOLDER, f)) if f.endswith('.pdf') else extract_text_from_docx(os.path.join(UPLOAD_FOLDER, f)) for f in os.listdir(UPLOAD_FOLDER)]
                similarity_score = detect_plagiarism(uploaded_text, existing_texts)
                ai_percentage = classify_text(uploaded_text)
                classification_result = f"{ai_percentage:.2f}% AI-generated"
            else:
                return jsonify({'error': 'Tidak ada teks yang dapat diproses'}), 400
        else:
            return jsonify({'error': 'File harus berupa PDF atau DOCX'}), 400
    
    return render_template('index.html', similarity=similarity_score, classification=classification_result)
